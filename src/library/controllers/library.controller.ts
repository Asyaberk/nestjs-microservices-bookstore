...
export class LibraryController {
  ...
  @ApiOkResponse({
    ...
    schema: {
      example: [
        { id: 1, title: 'Atomic Habits', author: 'James Clear', publishedYear: 2018, available: true },
        { id: 2, title: 'Clean Code', author: 'Robert C. Martin', publishedYear: 2008, available: true }
      ]
    }
  })
  ...
  @ApiOkResponse({
    ...
    schema: {
      example: [
        { rentalId: 10, book: { id: 1, title: 'Clean Code', author: 'Robert C. Martin' }, rentDate: '2025-08-05T12:00:00.000Z' }
      ]
    }
  })
  @ApiBadRequestResponse({ description: 'Invalid data provided.' })
  ...
  @ApiCreatedResponse({
    ...
    schema: {
      example: { rentalId: 15, user: { id: 1, email: 'user@mail.com' }, book: { id: 2, title: 'Clean Code' }, rentDate: '2025-08-05T12:00:00.000Z' }
    }
  })
  @ApiBadRequestResponse({ description: 'Book not found or not available.' })
  rentBook(@Req() req, @Body() dto: CreateRentalDto) {
    return this.libraryService.rent(req.user.id, dto);
  }
  ...
  @ApiParam({ name: 'bookId', example: 2, description: 'ID of the book to return' })
  @ApiOkResponse({
    description: 'Book returned successfully',
    schema: {
      example: { message: 'Book with ID 2 has been returned and is now available.' }
    }
  })
  @ApiBadRequestResponse({ description: 'Invalid book ID or rental not found.' })
  returnBook(@Req() req, @Param('bookId') bookId: string) {
    return this.libraryService.returnBook(req.user.id, Number(bookId));
  }
  ...
  @ApiOkResponse({
    ...
    schema: {
      example: [
        { rentalId: 10, user: { id: 1, email: 'user@mail.com' }, book: { id: 2, title: 'Clean Code' }, rentDate: '2025-08-05T12:00:00.000Z' },
        { rentalId: 11, user: { id: 2, email: 'another@mail.com' }, book: { id: 3, title: 'Atomic Habits' }, rentDate: '2025-08-06T12:00:00.000Z' }
      ]
    }
  })
